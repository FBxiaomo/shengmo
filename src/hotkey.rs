...
mod tests {
    ...
    fn shared_with_held_latches() -> Shared {
        Shared {
            ...
            press_dispatched: AtomicBool::new(true),
            press_down_at: std::sync::atomic::AtomicU64::new(0),
            press_cancelled: AtomicBool::new(false),
            press_generation: std::sync::atomic::AtomicU64::new(0),
...
struct Shared {
    ...
    /// 当前这次按住是否已经向 coordinator 派发过 Pressed 事件。
    /// 防止 auto-repeat 期间反复发送 Pressed。
    press_dispatched: AtomicBool,
    /// 首次 key-down 到达时的时间戳（毫秒）。用于 HOLD_THRESHOLD_MS 延迟识别。
    press_down_at: std::sync::atomic::AtomicU64,
    /// 定时器取消标记。WM_KEYUP 时置 true，定时器线程检查此标记决定是否放弃派发。
    press_cancelled: AtomicBool,
    /// 按键世代计数器。每次首次 key-down 递增，定时器线程启动时快照；
    /// 到期后对比当前值，不一致说明已经过 key-up + 新 key-down，旧定时器作废。
    press_generation: std::sync::atomic::AtomicU64,
    ...
    F: FnOnce(Arc<Shared>, Sender<HotkeyEvent>, StartupTx<T>) + Send + 'static,
{
    let shared = Arc::new(Shared {
        ...
        press_dispatched: AtomicBool::new(false),
        press_down_at: std::sync::atomic::AtomicU64::new(0),
        press_cancelled: AtomicBool::new(false),
        press_generation: std::sync::atomic::AtomicU64::new(0),
        qa_trigger: RwLock::new(None),
...
mod platform {
    ...
    /// Hold 模式延迟识别阈值（毫秒）。首次 key-down 后启动定时器，
    /// 到期时若键仍按住则向 coordinator 派发 Pressed 事件。
    /// 快速点按（定时器到期前松手）不触发任何事件。
    /// 测试环境设为 0 以消除时序依赖。
    #[cfg(not(test))]
    const HOLD_THRESHOLD_MS: u64 = 180;
    #[cfg(test)]
    const HOLD_THRESHOLD_MS: u64 = 0;
            ...
            WM_KEYDOWN | WM_SYSKEYDOWN => {
                ...
                if !was_held {
                    ...
                    let gen = ctx
                        ...
                        + 1;
                    ctx.shared.press_cancelled.store(false, Ordering::SeqCst);
                    ctx.shared.press_dispatched.store(false, Ordering::SeqCst);
                    log::info!(
                        "[hotkey] Windows trigger pressed vk={vk_code} — timer {HOLD_THRESHOLD_MS}ms gen={gen}"
                    );

                    if HOLD_THRESHOLD_MS == 0 {
                        // 测试环境：立即派发，不启动定时器线程。
                        ctx.shared.press_dispatched.store(true, Ordering::SeqCst);
                        send_or_log(&ctx.tx, HotkeyEvent::Pressed);
                    } else {
                        let shared = Arc::clone(&ctx.shared);
                        let tx = ctx.tx.clone();
                        std::thread::Builder::new()
                            .name("openless-hold-timer".into())
                            .spawn(move || {
                                std::thread::sleep(std::time::Duration::from_millis(
                                    HOLD_THRESHOLD_MS,
                                ));
                                // 世代检查：如果 generation 已变，说明经过了
                                // key-up → 新 key-down，旧定时器作废。
                                if shared.press_generation.load(Ordering::SeqCst) != gen {
                                    return;
                                }
                                // 取消检查：用户已在阈值内松手。
                                if shared.press_cancelled.load(Ordering::SeqCst) {
                                    return;
                                }
                                // 键仍按住 → 派发 Pressed。
                                shared
                ...
                // was_held == true → auto-repeat，定时器已在运行，忽略。
                true // 消费 key-down：目标应用不会收到触发键
            }
            WM_KEYUP | WM_SYSKEYUP => {
                // 取消定时器（可能还在 sleep 中）。
                ctx.shared.press_cancelled.store(true, Ordering::SeqCst);
                let _was_held = ctx.shared.trigger_held.swap(false, Ordering::SeqCst);
                let was_dispatched =
                    ctx.shared.press_dispatched.swap(false, Ordering::SeqCst);
                if was_dispatched {
                    // 定时器已派发过 Pressed，正常释放。
                    log::info!("[hotkey] Windows trigger released vk={vk_code}");
                    send_or_log(&ctx.tx, HotkeyEvent::Released);
                }
                // was_dispatched == false → 快速点按（定时器未到期），静默丢弃。
                // 放行 key-up（不消费），让 Windows 正常更新键盘状态。
                false
            }
            _ => true,
        }
