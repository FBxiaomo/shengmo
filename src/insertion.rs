...
fn insert_with_clipboard_restore(
    ...
    if let Err(err) = simulate_paste(paste_shortcut) {
        log::warn!("[insertion] simulated paste failed: {}", err);
        // Even when paste fails, the text is already on the clipboard.
        // If restore is requested, schedule it so the user's original
        // clipboard content is recovered after the paste window.
