...
impl VolcengineStreamingASR {
    ...
    /// 注入实时草稿通道。设置后，每次 ASR 收到 partial / final 文本时会通过
    /// 此 channel 发送 `(accumulated_text, is_partial)` 给前端。
    ...
    fn handle_frame(&self, data: &[u8]) -> bool {
        ...
        if !has_final && !full_text.is_empty() {
            let draft_tx = self.state.lock().draft_tx.clone();
            self.state.lock().last_partial_text = full_text.clone();
            // 发送实时草稿更新（partial）
            if let Some(ref tx) = draft_tx {
                let _ = tx.send((full_text.clone(), true));
            }
        }
        ...
        if has_final {
            ...
            let draft_tx = self.state.lock().draft_tx.clone();
            if let Some(ref tx) = draft_tx {
                let _ = tx.send((full_text.clone(), false));
            }
