...
impl WhisperBatchASR {
    ...
    pub async fn transcribe(&self) -> Result<RawTranscript> {
        ...
        if pcm.len() < original_len {
            log::info!(
                "[strip-silence] whisper: {original_len} → {} bytes ({}ms → {}ms)",
                pcm.len(),
                original_len / 32,
                pcm.len() / 32,
            );
...
mod tests {
    ...
    fn split_pcm_by_duration_uses_sample_boundaries() {
        let pcm = crate::asr::pcm::make_pcm_with_rms(32_000 * 65, 0.05);
    ...
    async fn openrouter_format_posts_json_with_base64_audio() {
        ...
        let pcm = crate::asr::pcm::make_pcm_with_rms(32_000 * 2, 0.05);
        asr.consume_pcm_chunk(&pcm);
