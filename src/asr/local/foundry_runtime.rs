...
mod imp {
    ...
    /// Wrap a Foundry SDK async call in `tokio::task::spawn` so a Rust panic
    /// inside the native SDK is caught as a `JoinError` instead of aborting
    /// the process.  Use this for every `foundry_local_sdk` method that may
    /// call into native (C++/WinRT) code.
    async fn catch_sdk_panic<F, T, E>(fut: F, what: &str) -> Result<T>
    where
        ...
        E: std::fmt::Display + Send + 'static,
    {
        ...
        match tokio::task::spawn(fut).await {
            ...
            Ok(Err(sdk_err)) => anyhow::bail!("{what}: {sdk_err}"),
            Err(join_err) => {
                if join_err.is_panic() {
                    anyhow::bail!("Foundry Local SDK 在 {what} 时崩溃");
                }
                Err(anyhow::Error::new(join_err).context(what))
            }
        }
    }
    ...
    impl FoundryLocalRuntime {
        ...
        pub async fn catalog_snapshot(&self) -> Result<Vec<FoundryCatalogModel>> {
            ...
            for known in MODELS {
                ...
                catalog.push(FoundryCatalogModel {
                    ...
                    quality_tier: known.quality_tier.to_string(),
                    cached,
        ...
        async fn ensure_loaded_locked(
            ...
            catch_sdk_panic(
                manager.download_and_register_eps_with_progress(
                    None,
            ...
            if let Err(error) = {
                ...
                catch_sdk_panic(
                    async move { load_model.load().await },
                    &format!("加载 Foundry 模型 {alias}"),
                )
                .await
            ...
            if self.cancel_prepare.load(Ordering::SeqCst) {
                if let Err(error) = {
                    let unload_model = Arc::clone(&model);
                    catch_sdk_panic(
                        async move { unload_model.unload().await },
                        &format!("卸载已取消的 Foundry 模型 {alias}"),
                    )
                    .await
        ...
        async fn restore_loaded_model(
            ...
            let restore = Arc::clone(&loaded.model);
            catch_sdk_panic(
                async move { restore.load().await },
                &format!("恢复 Foundry 模型 {}", loaded.model_id),
            )
            .await
        ...
        fn manager(&self) -> Result<&'static FoundryLocalManager> {
            ...
            let config = self.manager_config();
            let manager =
                ...
                .map_err(|_| {
                    anyhow::anyhow!(
                        "Foundry Local SDK 初始化崩溃，请确认 Windows App Runtime 已安装"
                    )
                })?
        ...
        async fn unload_model(loaded: &LoadedModel) -> Result<()> {
            let unload = Arc::clone(&loaded.model);
            catch_sdk_panic(
                async move { unload.unload().await },
                &format!("卸载 Foundry 模型 {}", loaded.model_id),
            )
            .await
