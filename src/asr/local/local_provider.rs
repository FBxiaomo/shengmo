...
impl LocalQwenAsr {
    ...
    pub async fn transcribe(self: Arc<Self>) -> Result<RawTranscript> {
        ...
        // 裁剪前后静音，减少本地推理时长。
        let original_len = pcm_bytes.len();
        let pcm_bytes = crate::asr::pcm::strip_silence(&pcm_bytes, 3200, 0.01, 6400);
        if pcm_bytes.len() < original_len {
            log::info!(
                "[strip-silence] qwen3: {original_len} → {} bytes ({}ms → {}ms)",
                pcm_bytes.len(),
                original_len / 32,
                pcm_bytes.len() / 32,
            );
