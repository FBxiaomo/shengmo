...
/// 新增：Whisper Large V3 高质量多语、Paraformer 中文流式、Zipformer Small CTC 中文流式。
#[allow(dead_code)]
pub const MODELS: &[SherpaModel] = &[
    ...
    SherpaModel {
        ...
        display_name: "Paraformer Streaming (zh)",
        family: SherpaFamily::Paraformer,
        mode: SherpaMode::Online,
        languages: &["zh"],
        quality_tier: "streaming-chinese",
    },
    SherpaModel {
        alias: "zipformer-small-ctc-zh-streaming",
        display_name: "Zipformer Small CTC Streaming (zh)",
        family: SherpaFamily::Zipformer,
        mode: SherpaMode::Online,
        languages: &["zh"],
        quality_tier: "streaming-ctc",
    },
];
...
pub fn hf_repo_for_alias(alias: &str) -> Result<&'static str> {
    match alias {
        ...
        "paraformer-streaming-zh" => Ok("csukuangfj/sherpa-onnx-streaming-paraformer-zh"),
        "zipformer-small-ctc-zh-streaming" => {
            Ok("csukuangfj/sherpa-onnx-streaming-zipformer-small-ctc-zh-int8-2025-04-01")
        }
...
pub fn required_files_for_alias(alias: &str) -> Result<&'static [&'static str]> {
    match alias {
        ...
        "paraformer-streaming-zh" => Ok(&["encoder.int8.onnx", "decoder.int8.onnx", "tokens.txt"]),
        "zipformer-small-ctc-zh-streaming" => Ok(&["model.int8.onnx", "tokens.txt"]),
        _ => anyhow::bail!("unknown sherpa-onnx model alias: {alias}"),
    }
}
...
pub fn release_archive_for_alias(_alias: &str) -> Option<SherpaReleaseArchive> {
    // Qwen3 migrated to ModelScope file-by-file download (see modelscope_files_for_alias).
    None
}
...
pub struct ModelScopeFile {
    /// 魔搭仓库内路径（如 `model_0.6B/conv_frontend.onnx`）
    pub remote_path: &'static str,
    /// 本地模型目录内路径（如 `conv_frontend.onnx`）
...
/// 返回魔搭社区仓库 ID 和文件列表（仅对 Qwen3 系列生效）。
pub fn modelscope_files_for_alias(alias: &str) -> Option<(&'static str, &'static [ModelScopeFile])> {
    match alias {
        "qwen3-asr-0.6b-int8" => Some((
            "zengshuishui/Qwen3-ASR-onnx",
            &[
                ModelScopeFile {
                    remote_path: "model_0.6B/conv_frontend.onnx",
                    local_path: "conv_frontend.onnx",
                    size: 44_148_281,
                    sha256: Some("d22dc4423e0940e49884e903d2ea2f7e5567c14fc1aed97e4e26d6b8f208ef9e"),
                },
                ModelScopeFile {
                    remote_path: "model_0.6B/encoder.int8.onnx",
                    local_path: "encoder.int8.onnx",
                    size: 182_491_662,
                    sha256: Some("60748d3e6744a57c9c91e1b17424a6c2990567e8adceb0783940c03ed98fa9d9"),
                },
                ModelScopeFile {
                    remote_path: "model_0.6B/decoder.int8.onnx",
                    local_path: "decoder.int8.onnx",
                    size: 755_914_231,
                    sha256: Some("4f6885be5959ae26af3089d38ee7972c5fafbeeb1cf8d5e76eab6d8b61ca5771"),
                },
                ModelScopeFile {
                    remote_path: "tokenizer/vocab.json",
                    local_path: "tokenizer/vocab.json",
                    size: 2_776_833,
                    sha256: None,
                },
                ModelScopeFile {
                    remote_path: "tokenizer/merges.txt",
                    local_path: "tokenizer/merges.txt",
                    size: 1_671_853,
                    sha256: None,
                },
                ModelScopeFile {
                    remote_path: "tokenizer/config.json",
                    local_path: "tokenizer/config.json",
                    size: 6_193,
                    sha256: None,
                },
                ModelScopeFile {
...
impl SherpaCatalogModel {
    ...
    pub fn from_static(model: &SherpaModel) -> Self {
        Self {
            ...
            quality_tier: model.quality_tier.to_string(),
            cached: false,
            downloaded_bytes: 0,
            file_size_mb: None,
        }
...
mod tests {
    ...
    fn static_catalog_preserves_ui_order() {
        ...
        // 最后三个是流式模型
        assert_eq!(catalog[5].mode, SherpaMode::Online);
        assert_eq!(catalog[6].mode, SherpaMode::Online);
        assert_eq!(catalog[7].mode, SherpaMode::Online);
    }
