...
pub async fn fetch_remote_info(model_alias: &str, mirror: Mirror) -> Result<SherpaRemoteInfo> {
    ...
    // 魔搭社区逐文件下载：直接用硬编码的文件信息，不需要调 tree API。
    if let Some((_repo, ms_files)) = sherpa::modelscope_files_for_alias(model_alias) {
        let files: Vec<SherpaRemoteFile> = ms_files
            .iter()
            .map(|f| SherpaRemoteFile {
                path: f.remote_path.to_string(),
                local_path: f.local_path.to_string(),
                size: f.size,
                sha256: f.sha256.map(str::to_string),
            })
            .collect();
        let total_bytes = files.iter().map(|f| f.size).sum();
        return Ok(SherpaRemoteInfo {
            model_alias: model_alias.to_string(),
            mirror: "modelscope".to_string(),
            files,
            total_bytes,
        });
    }
    ...
    .await
    .with_context(|| format!("HF tree API 超时 (30s): {url}"))?
...
async fn run_download(
    ...
    let is_modelscope = sherpa::modelscope_files_for_alias(model_alias).is_some();
    let repo = if is_modelscope {
        sherpa::modelscope_files_for_alias(model_alias).unwrap().0
    } else {
        sherpa::hf_repo_for_alias(model_alias)?
    };
    ...
    for (idx, file) in info.files.iter().cloned().enumerate() {
        ...
        let url = if is_modelscope {
            format!("https://modelscope.cn/models/{}/resolve/master/{}", repo, file.path)
        } else {
            format!("{}/{}/resolve/main/{}", mirror.base_url(), repo, file.path)
        };
