...
impl SherpaOnnxAsr {
    ...
    async fn transcribe_offline(
        ...
        let pcm = crate::asr::pcm::strip_silence(&pcm, 3200, 0.01, 6400);
        if pcm.len() < original_len {
            log::info!(
                "[strip-silence] sherpa: {original_len} → {} bytes ({}ms → {}ms)",
                pcm.len(),
                original_len / 32,
                pcm.len() / 32,
            );
...
impl OnlineWorker {
    fn spawn(
        mut session: SherpaOnlineSession,
        token_handler: Option<SherpaTokenHandler>,
    ) -> Result<Self> {
        ...
        let join_handle = std::thread::Builder::new()
            ...
            .spawn(move || {
                            ...
                            Ok(OnlineWorkerMessage::Pcm(pcm)) => {
                                if worker_cancelled.load(Ordering::SeqCst) {
                                    break Err(anyhow::anyhow!(
                                        "sherpa-onnx streaming cancelled"
                                    ));
                                }
                ...
                let final_result = match result {
                    ...
                    Err(_) => {
                        log::error!(
                            "[sherpa-asr] online worker thread panicked (native crash recovered)"
                        );
                        Err(anyhow::anyhow!(
                            "sherpa-onnx worker thread panicked (native crash recovered)"
                        ))
                    }
                };
                let _ = result_tx.send(final_result);
            })
            .context("spawn sherpa online worker")?;
...
mod tests {
    ...
    async fn transcribe_clears_buffer_on_runtime_error() {
        ...
        provider.consume_pcm_chunk(&crate::asr::pcm::make_pcm_with_rms(32_000, 0.05));
