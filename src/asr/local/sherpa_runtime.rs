...
use crate::asr::local::sherpa::{
    ...
    SherpaPrepareProgressPayload, SherpaRuntimeStatus, DEFAULT_ONLINE_MODEL_ALIAS, PROVIDER_ID,
};
...
#[cfg(target_os = "windows")]
fn validate_model_files(alias: &str, dir: &Path) -> Result<()> {
    let required: Vec<&str> = match alias {
        "paraformer-streaming-zh" => vec!["encoder.int8.onnx", "decoder.int8.onnx", "tokens.txt"],
        "zipformer-small-ctc-zh-streaming" => vec!["model.int8.onnx", "tokens.txt"],
        _ => {
            let family = model_family(alias)?;
            match family {
                SherpaFamily::SenseVoice | SherpaFamily::Paraformer => {
                    vec!["model.int8.onnx", "tokens.txt"]
                }
                SherpaFamily::Whisper => {
                    vec!["encoder.int8.onnx", "decoder.int8.onnx", "tokens.txt"]
                }
    ...
    for name in required {
        ...
        if !path.exists() {
            anyhow::bail!("模型文件缺失: {} (模型 {})", name, alias);
        }
        if !path.is_dir() {
...
async fn load_model(alias: &str, dir: &Path) -> Result<LoadedModel> {
    ...
    tokio::task::spawn_blocking(move || {
        validate_model_files(&alias, &dir)?;
...
fn create_offline_recognizer(alias: &str, dir: &Path) -> Result<OfflineRecognizer> {
    ...
    match model_family(alias)? {
        SherpaFamily::SenseVoice => {
            ...
            log::info!(
                "[sherpa-onnx] SenseVoice config: dir={}, tokens={} ({}), model={} ({})",
                dir.display(),
                tokens_path.display(),
                file_size_str(&tokens_path),
                model_path.display(),
                file_size_str(&model_path),
            );
            config.model_config.tokens = Some(path_to_string(&tokens_path)?);
            config.model_config.sense_voice = OfflineSenseVoiceModelConfig {
                model: Some(path_to_string(&model_path)?),
        ...
        SherpaFamily::Paraformer => {
            ...
            let model_path = dir.join("model.int8.onnx");
            log::info!(
                "[sherpa-onnx] Paraformer config: dir={}, tokens={}, model={}",
                dir.display(),
                tokens_path.display(),
                model_path.display(),
            );
        ...
        SherpaFamily::Whisper => {
            ...
            log::info!(
                "[sherpa-onnx] Whisper config: dir={}, tokens={}, encoder={}, decoder={}",
                dir.display(),
                tokens_path.display(),
            ...
            config.model_config.whisper = OfflineWhisperModelConfig {
                encoder: Some(path_to_string(&encoder_path)?),
                decoder: Some(path_to_string(&decoder_path)?),
                language: Some("auto".into()),
        ...
        SherpaFamily::Qwen3Asr => {
            let conv_path = dir.join("conv_frontend.onnx");
            let encoder_path = dir.join("encoder.int8.onnx");
            let decoder_path = dir.join("decoder.int8.onnx");
            let tokenizer_path = dir.join("tokenizer");
            log::info!(
                "[sherpa-onnx] Qwen3ASR config: dir={}, conv={}, encoder={}, decoder={}, tokenizer={}",
                dir.display(),
                conv_path.display(),
                encoder_path.display(),
                decoder_path.display(),
            ...
            config.model_config.qwen3_asr = OfflineQwen3ASRModelConfig {
                conv_frontend: Some(path_to_string(&conv_path)?),
                encoder: Some(path_to_string(&encoder_path)?),
                decoder: Some(path_to_string(&decoder_path)?),
                tokenizer: Some(path_to_string(&tokenizer_path)?),
                ..Default::default()
            };
    ...
    log::info!(
        "[sherpa-onnx] creating OfflineRecognizer for alias={alias}, dir={}, threads={}, provider=cpu, debug=true",
        dir.display(),
    ...
    .map_err(|_| {
        anyhow::anyhow!(
            "sherpa-onnx OfflineRecognizer::create panicked (alias={alias}, dir={})",
            dir.display()
        )
    })?
    .ok_or_else(|| {
        anyhow::anyhow!(
            "create sherpa-onnx offline recognizer failed (alias={alias}, dir={}, 请检查模型文件是否完整或版本是否兼容)",
            dir.display()
        )
    })?;
    log::info!("[sherpa-onnx] OfflineRecognizer created successfully for alias={alias}");
    Ok(recognizer)
}
...
fn create_online_recognizer(alias: &str, dir: &Path) -> Result<OnlineRecognizer> {
    ...
    match alias {
        // ── Zipformer transducer（双语流式） ──────────────────────────
        a if a == DEFAULT_ONLINE_MODEL_ALIAS => {
            let tokens_path = dir.join("tokens.txt");
            let encoder_path = dir.join("encoder-epoch-99-avg-1.int8.onnx");
            let decoder_path = dir.join("decoder-epoch-99-avg-1.onnx");
            let joiner_path = dir.join("joiner-epoch-99-avg-1.int8.onnx");
            log::info!(
                "[sherpa-onnx] Zipformer transducer online config: dir={}, tokens={} ({}), encoder={} ({}), decoder={} ({}), joiner={} ({})",
                dir.display(),
                tokens_path.display(), file_size_str(&tokens_path),
                encoder_path.display(), file_size_str(&encoder_path),
                decoder_path.display(), file_size_str(&decoder_path),
                joiner_path.display(), file_size_str(&joiner_path),
            );
            config.model_config.tokens = Some(path_to_string(&tokens_path)?);
            ...
            config.model_config.transducer.decoder =
                Some(path_to_string(&decoder_path)?);
            config.model_config.transducer.joiner = Some(path_to_string(
                &joiner_path,
            )?);
        }
        // ── Zipformer CTC（中文流式） ───────────────────────────────
        "zipformer-small-ctc-zh-streaming" => {
            let tokens_path = dir.join("tokens.txt");
            let model_path = dir.join("model.int8.onnx");
            log::info!(
                "[sherpa-onnx] Zipformer CTC online config: dir={}, tokens={} ({}), model={} ({})",
                dir.display(),
                tokens_path.display(), file_size_str(&tokens_path),
                model_path.display(), file_size_str(&model_path),
            );
            config.model_config.tokens = Some(path_to_string(&tokens_path)?);
            config.model_config.zipformer2_ctc.model = Some(path_to_string(&model_path)?);
        }
        // ── Paraformer 流式（中文） ────────────────────────────────
        "paraformer-streaming-zh" => {
            let tokens_path = dir.join("tokens.txt");
            let encoder_path = dir.join("encoder.int8.onnx");
            let decoder_path = dir.join("decoder.int8.onnx");
            log::info!(
                "[sherpa-onnx] Paraformer streaming config: dir={}, tokens={} ({}), encoder={} ({}), decoder={} ({})",
                dir.display(),
                tokens_path.display(), file_size_str(&tokens_path),
                encoder_path.display(), file_size_str(&encoder_path),
                decoder_path.display(), file_size_str(&decoder_path),
        ...
        other => anyhow::bail!("sherpa-onnx alias {other} is not supported by online ASR"),
    }
    log::info!(
        "[sherpa-onnx] creating OnlineRecognizer for alias={alias}, dir={}, threads={}",
        dir.display(),
        config.model_config.num_threads,
    );
    let recognizer = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
    ...
    .map_err(|_| {
        anyhow::anyhow!(
            "sherpa-onnx OnlineRecognizer::create panicked (alias={alias}, dir={})",
            dir.display()
        )
    })?
    .ok_or_else(|| {
        anyhow::anyhow!(
            "create sherpa-onnx online recognizer failed (alias={alias}, dir={}, 请检查模型文件是否完整或版本是否兼容)",
            dir.display()
        )
    })?;
    log::info!("[sherpa-onnx] OnlineRecognizer created successfully for alias={alias}");
    Ok(recognizer)
}
...
async fn transcribe_loaded_model(
    ...
    tokio::time::timeout(audio_timeout, async move {
        tokio::task::spawn_blocking(move || {
            ...
            .unwrap_or_else(|_| {
                Err(anyhow::anyhow!(
                    "sherpa-onnx transcribe panicked (native crash recovered)"
                ))
            })
        })
        .await
