...
struct SyncState {
    ...
    /// 实时草稿回调：每次 partial / final 文本更新时发送 (text, is_partial)。
    draft_tx: Option<mpsc::UnboundedSender<(String, bool)>>,
}
...
impl BailianRealtimeASR {
    ...
    /// 注入实时草稿通道。
    pub fn set_draft_tx(&self, tx: mpsc::UnboundedSender<(String, bool)>) {
        self.state.lock().draft_tx = Some(tx);
    }

    /// 返回当前累积的识别文本（final segments + 最新 partial）。
    pub fn get_accumulated_text(&self) -> String {
        let st = self.state.lock();
        let base = if st.final_segments.is_empty() {
            String::new()
        } else {
            let segments: Vec<String> = st.final_segments.values().cloned().collect();
            merge_segments(&segments)
        };
        // 取最大的 sentence_id 对应的 partial（BTreeMap 有序，last_key_value 即最新）
        let partial_tail = st
            .partial_segments
            .last_key_value()
            .map(|(_, v)| v.as_str())
    ...
    fn record_result(&self, value: &Value) {
        ...
        // 计算累积文本并发送实时草稿更新
        let accumulated = if st.final_segments.is_empty() {
            st.last_result_text.clone()
        } else {
            let segs: Vec<String> = st.final_segments.values().cloned().collect();
            let mut base = merge_segments(&segs);
            if let Some((_, ptail)) = st.partial_segments.last_key_value() {
                base.push_str(ptail);
            }
            base
        };
        let draft_tx = st.draft_tx.clone();
        drop(st);

        if let Some(ref tx) = draft_tx {
            let _ = tx.send((accumulated, !is_sentence_final));
        }
    }
