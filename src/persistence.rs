...
const LEGACY_CREDS_DIR: &str = ".shengmo";
...
fn data_dir() -> Result<PathBuf> {
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        Ok(PathBuf::from(appdata).join("ShengMo"))
    }
    ...
    #[cfg(all(unix, not(target_os = "macos"), not(target_os = "android")))]
    {
        if let Ok(xdg) = std::env::var("XDG_DATA_HOME") {
            if !xdg.is_empty() {
                return Ok(PathBuf::from(xdg).join("ShengMo"));
            }
        }
        let home = std::env::var("HOME").context("HOME not set")?;
        Ok(PathBuf::from(home)
    ...
    #[cfg(target_os = "android")]
    {
        if let Ok(dir) = std::env::var("TAURI_ANDROID_APP_DATA_DIR") {
            return Ok(PathBuf::from(dir).join("ShengMo"));
        }
        Ok(std::env::temp_dir().join("ShengMo"))
    }
}
...
pub fn models_root_for_base_dir(base_dir: Option<&str>) -> Result<PathBuf> {
    ...
    let dir = match trimmed {
        Some(base) => PathBuf::from(base).join("ShengMo").join("models"),
...
impl CredentialsVault {
    ...
    pub const SERVICE_NAME: &'static str = "com.shengmo.app";
    ...
    /// 遍历所有已存储的 ASR 提供商，返回每个提供商是否已配置完整凭据。
    pub fn all_asr_providers_configured() -> HashMap<String, bool> {
        let _guard = credentials_lock().lock();
        let root = load_credentials();
        let has = |s: &Option<String>| s.as_ref().map(|v| !v.trim().is_empty()).unwrap_or(false);
        let mut map = HashMap::new();
        for (id, entry) in &root.providers.asr {
            let ok = if id == "volcengine" {
                (has(&entry.appKey) || has(&entry.apiKey))
                    && has(&entry.accessKey)
                    && has(&entry.resourceId)
            } else if id == crate::asr::bailian::PROVIDER_ID {
                has(&entry.apiKey)
            } else {
                // mimo / whisper-compatible / 其他云端
                has(&entry.apiKey) && has(&entry.baseURL) && has(&entry.model)
            };
            map.insert(id.clone(), ok);
        }
        map
    }

    /// 遍历所有已存储的 LLM 提供商，返回每个提供商是否已配置完整凭据。
    pub fn all_llm_providers_configured() -> HashMap<String, bool> {
        let _guard = credentials_lock().lock();
        let root = load_credentials();
        let has = |s: &Option<String>| s.as_ref().map(|v| !v.trim().is_empty()).unwrap_or(false);
        let mut map = HashMap::new();
        for (id, entry) in &root.providers.llm {
            let ok = if id == crate::polish::CODEX_OAUTH_PROVIDER_ID {
                crate::polish::CodexOAuthCredentials::load_default().is_ok()
            } else if id == "custom" {
                has(&entry.apiKey) && has(&entry.baseURL) && has(&entry.model)
            } else {
                has(&entry.apiKey)
...
mod tests {
    ...
    fn custom_models_root_uses_openless_models_suffix() {
        ...
        assert_eq!(root, tmp.join("ShengMo").join("models"));
        assert!(root.is_dir());
