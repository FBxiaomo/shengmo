...
export type VoiceOutputMode = 'smart' | 'cursor_only' | 'clipboard_only';
...
export interface UserPreferences {
  ...
  lastCloudAsrProvider: string;
  activeLlmProvider: string;
  /** LLM 思考模式开关。默认关闭；OpenAI 普通 chat 模型会跳过不支持的字段。详见 issue #402。 */
  llmThinkingEnabled: boolean;
  /** LLM 润色开关。默认 false（关闭润色）。仅在 normalMode = false 时生效。 */
  polishEnabled: boolean;
  /** 离线识别总开关。默认 true。关闭后首页不显示离线模型选项。 */
  offlineEnabled: boolean;
  /** 普通模式：纯语音转文字，跳过所有 AI 后处理。默认 true。 */
  normalMode: boolean;
  /** LLM 润色超时秒数。默认 8 秒。UI 上可通过 Overview 页面的步进器调整。 */
  polishTimeoutSecs: number;
  ...
  /** 语音输出模式：控制识别结果的输出策略。默认 'smart'（智能模式）。 */
  ...
  /** 迷你模式切换的全局快捷键。null = 用户已停用。默认 Ctrl+Shift+L。 */
  toggleMiniHotkey: ShortcutBinding | null;
...
export interface CapsulePayload {
  ...
  operating?: boolean;
  /** LLM 润色超时回退到 ASR 原文时为 true，胶囊显示"快速模式"徽标。 */
  quickMode?: boolean;
  /** ASR 模型是否为流式识别。流式模型显示单行草稿+左渐变遮罩+右侧小波形。 */
  isStreaming?: boolean;
}

export interface AsrDraftUpdate {
  /** 流式 ASR 累积的草稿文本（已确认片段 + 当前 partial）。 */
  text: string;
...
export interface CredentialsStatus {
  ...
  /** 每个 ASR 提供商是否已配置凭据（provider_id → configured）。 */
  asrProvidersConfigured: Record<string, boolean>;
  /** 每个 LLM 提供商是否已配置凭据（provider_id → configured）。 */
  llmProvidersConfigured: Record<string, boolean>;
}
