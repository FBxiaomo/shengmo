...
let mockSettings: UserPreferences = {
    ...
    dictationHotkey: { primary: "M", modifiers: ["ctrl", "shift"] },
    ...
    lastCloudAsrProvider: "volcengine",
    activeLlmProvider: "ark",
    llmThinkingEnabled: false,
    polishEnabled: false,
    offlineEnabled: true,
    normalMode: true,
    polishTimeoutSecs: 20,
    restoreClipboardAfterPaste: true,
    pasteShortcut: "ctrlV",
    allowNonTsfInsertionFallback: true,
    voiceOutputMode: 'smart' as const,
    ...
    qaSaveHistory: false,
    customComboHotkey: { primary: "M", modifiers: ["ctrl", "shift"] },
    ...
    toggleMiniHotkey: { primary: "L", modifiers: defaultAppShortcutModifiers() },
...
const mockCredentialsStatus: CredentialsStatus = {
    ...
    arkConfigured: true,
    asrProvidersConfigured: { volcengine: true, bailian: true },
    llmProvidersConfigured: { ark: true, deepseek: true },
}
...
export function enterMiniMode(): Promise<void> {
    return invokeOrMock("enter_mini_mode", undefined, () => undefined)
}

// 退出迷你模式：隐藏迷你浮窗，恢复主窗口。
export function exitMiniMode(): Promise<void> {
    return invokeOrMock("exit_mini_mode", undefined, () => undefined)
}
...
export function setToggleMiniHotkey(binding: ShortcutBinding | null): Promise<void> {
    return invokeOrMock("set_toggle_mini_hotkey", { binding }, () => undefined)
}
...
export function getProcessMemory(): Promise<ProcessMemoryInfo> {
    return invokeOrMock<ProcessMemoryInfo>(
        "get_process_memory",
        undefined,
        () => ({
            memoryBytes: 156_000_000,
            virtualBytes: 280_000_000,
            platform: 'windows',
        }),
    )
}
...
export function getGpuMemory(): Promise<GpuMemoryInfo> {
    return invokeOrMock<GpuMemoryInfo>(
        "get_gpu_memory",
        undefined,
        () => ({
            usedBytes: 82_000_000,
            available: true,
        }),
    )
}
