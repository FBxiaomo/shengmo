...
async fn validate_llm_provider() -> Result<(), String> {
    ...
    // Gemini 使用原生 generateContent API（x-goog-api-key 认证），
    // 不兼容 OpenAI /chat/completions 格式，必须走 GeminiProvider。
    #[cfg(not(mobile))]
    if CredentialsVault::get_active_llm() == "gemini" {
        let (api_key, model, base_url) = crate::coordinator::read_gemini_credentials()
            .map_err(|e| e.to_string())?;
...
async fn validate_asr_provider() -> Result<(), String> {
    ...
    // 火山引擎使用独立的凭据体系（AppKey/AccessKey）和 WebSocket 流式协议，
    // 不能走 Whisper 兼容的 HTTP /audio/transcriptions 验证路径。
    if active_asr == "volcengine" {
        return validate_volcengine_asr_provider();
    }
...
fn validate_volcengine_asr_provider() -> Result<(), String> {
    let app_key = CredentialsVault::get(CredentialAccount::VolcengineAppKey)
        .map_err(|e| e.to_string())?
        .unwrap_or_default();
    let access_key = CredentialsVault::get(CredentialAccount::VolcengineAccessKey)
        .map_err(|e| e.to_string())?
        .unwrap_or_default();
    if app_key.trim().is_empty() || access_key.trim().is_empty() {
        return Err("请先在设置中填写火山引擎 ASR App Key 和 Access Key".to_string());
    }
    Ok(())
}
