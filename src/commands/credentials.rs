...
pub fn get_credentials() -> CredentialsStatus {
    ...
    let asr_providers_configured = CredentialsVault::all_asr_providers_configured();
    let llm_providers_configured = CredentialsVault::all_llm_providers_configured();
    CredentialsStatus {
        active_asr_provider,
        active_llm_provider,
        asr_configured,
...
pub(crate) fn asr_configured_for_provider(provider: &str, snap: &CredentialsSnapshot) -> bool {
    ...
    // Whisper 兼容提供商（whisper / siliconflow / zhipu / groq / openrouter）
    // 需要 api_key + endpoint + model 三项都填齐，与运行时 ensure_asr_credentials 对齐。
