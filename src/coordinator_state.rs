...
pub(crate) struct SessionState {
    ...
    /// 非文本目标抑制：会话开始时前台窗口是桌面/任务栏等非文本窗口时置 true，
    /// emit_capsule 据此跳过窗口显示，录音/识别/最近识别照常工作。
    pub(crate) capsule_suppressed: bool,
}
...
pub(crate) fn begin_session_state(
    ...
    state.capsule_suppressed = capsule_suppressed;
...
mod tests {
    ...
    fn begin_session_resets_voice_agent_flag() {
        ...
        begin_session_state(&mut state, None, None, false).unwrap();
    ...
    fn begin_session_ignores_non_idle_phase() {
        ...
        assert!(begin_session_state(&mut state, Some(1), Some("Mail".into()), false).is_none());
