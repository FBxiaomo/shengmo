...
你是声墨的润色助手。先理解用户意图，再把口语化的转写整理为顺畅、自然、可直接发送的文字。
...
export function Style() {
  ...
  const handleActivate = async (pack: StylePack) => {
    ...
    try {
      ...
      if (pack.id === BUILTIN_RAW_ID) {
        await updatePrefs(current => ({ ...current, polishEnabled: false }));
      } else {
        const creds = await getCredentials();
        if (!creds.llmConfigured) {
          const proceed = window.confirm(t('style.pack.llmNotConfiguredConfirm'));
          if (!proceed) {
        ...
        await updatePrefs(current => ({ ...current, polishEnabled: true, normalMode: false }));
      }
            ...
            {bodyPacks.map(pack => {
              ...
              return (
                <motion.div
                  ...
                  whileTap={isBuiltin ? undefined : { scale: 0.98 }}
                  onTap={isBuiltin ? undefined : () => openEditorForPack(pack)}
                  onClick={isBuiltin ? undefined : () => openEditorForPack(pack)}
                  style={{
                    display: 'flex',
                ...
                >
                  <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 10, marginBottom: 10 }}>
                    ...
                    {isBuiltin ? (
                      ...
                      <button
                        type="button"
                        onClick={e => { e.stopPropagation(); void handleDeleteImportedPack(pack); }}
                  ...
                  {(isBuiltin || pack.tags.length > 0) && (
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap', minHeight: 24, marginBottom: 12 }}>
                    <Pill tone={isBuiltin ? modeTone(pack.baseMode) : 'violet'} size="sm">{t(`style.modes.${pack.baseMode}.name`)}</Pill>
                    {pack.tags.slice(0, 1).map(tag => (
                      <Pill key={`${pack.id}-${tag}`} tone={isBuiltin ? 'default' : 'violet'} size="sm">{tag}</Pill>
                    ))}
                  </div>
                  )}

                  <div onClick={e => e.stopPropagation()} style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginTop: 'auto' }}>
                    <Btn
                      size="sm"
