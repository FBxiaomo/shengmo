    ...
    <>
      ...
      <Card>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, marginBottom: 10, flexWrap: 'wrap' }}>
          <SectionTitle style={{ marginBottom: 0 }}>{t('settings.providers.llmTitle')}</SectionTitle>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <SelectLite
              ...
              style={{ ...inputStyle, minWidth: 160, maxWidth: mobile ? '100%' : 200 }}
            />
            <span style={{ fontSize: 12, color: (prefs?.polishEnabled ?? false) ? 'var(--ol-blue)' : 'var(--ol-ink-4)' }}>
              {t('settings.providers.polishToggleLabel', '润色')}
            </span>
            <Toggle
              on={prefs?.polishEnabled ?? false}
              onToggle={enabled => void updatePrefs(current => ({ ...current, polishEnabled: enabled, normalMode: !enabled }))}
            />
          </div>
        </div>
        <div style={{ opacity: (prefs?.polishEnabled ?? false) ? 1 : 0.45, pointerEvents: (prefs?.polishEnabled ?? false) ? 'auto' : 'none' }}>
        {codexOAuthSelected ? (
        ...
        />
        <SettingRow label={t('settings.providers.timeoutLabel', '润色超时')}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 3 }}>
            <input
              type="text"
              inputMode="numeric"
              value={prefs?.polishTimeoutSecs ?? 20}
              onChange={e => {
                const raw = e.target.value.replace(/\D/g, '');
                const n = raw === '' ? 1 : Math.max(1, Math.min(60, parseInt(raw, 10)));
                void updatePrefs(current => ({ ...current, polishTimeoutSecs: n }));
              }}
              style={{
                width: 36, height: 26, fontSize: 12, textAlign: 'center',
                border: '0.5px solid var(--ol-line)', borderRadius: 4,
                background: 'var(--ol-bg)', color: 'var(--ol-ink)',
                outline: 'none', padding: 0, ...inputStyle,
              }}
            />
            <span style={{ fontSize: 11, color: 'var(--ol-ink-4)' }}>s</span>
            <button
              onClick={() => {
                const v = Math.max(1, (prefs?.polishTimeoutSecs ?? 20) - 1);
                void updatePrefs(current => ({ ...current, polishTimeoutSecs: v }));
              }}
              style={{
                width: 22, height: 22, border: '0.5px solid var(--ol-line)',
                borderRadius: 4, background: 'var(--ol-bg-soft)', color: 'var(--ol-ink-3)',
            ...
            <button
              onClick={() => {
                const v = Math.min(60, (prefs?.polishTimeoutSecs ?? 20) + 1);
                void updatePrefs(current => ({ ...current, polishTimeoutSecs: v }));
              }}
              style={{
                width: 22, height: 22, border: '0.5px solid var(--ol-line)',
      ...
      {showAsr && (() => {
        // 本地引擎激活时不在主下拉里——补一个可选 option 让 select 显示当前值并允许切走。
      ...
      <Card>
        <div style={{ marginBottom: 10 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
            <SectionTitle style={{ marginBottom: 0 }}>{t('settings.providers.asrTitle')}</SectionTitle>
            <SelectLite
              value={asrProvider}
          ...
          {hiddenLocalActive && (
            <div style={{ fontSize: 11, color: 'var(--ol-ink-4)', lineHeight: 1.5, marginTop: 4, textAlign: 'right' }}>
