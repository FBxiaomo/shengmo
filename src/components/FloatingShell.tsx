...
export function FloatingShell({ os: osProp, initialTab = 'overview', initialSettings = false }: FloatingShellProps) {
  ...
  return (
    <WindowChrome os={os} title="声墨" height="100%">
      ...
      <div
        ...
        <aside
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 9, padding: '2px 8px 12px' }}>
            ...
            <div style={{ fontSize: 13.5, fontWeight: 600, letterSpacing: '-0.01em', color: 'var(--ol-ink)' }}>声墨</div>
          </div>
        ...
        <div style={{ flex: 1, minWidth: 0, padding: mobile ? 0 : '4px 8px 8px 0', display: 'flex' }}>
          ...
          >
            ...
            >
              {displayTab === 'overview' ? (
                <Overview onOpenHistory={() => setCurrentTab('history')} onOpenSettings={() => openSettings('services')} />
              ) : (
                <Page />
              )}
            </div>
