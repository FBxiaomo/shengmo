...
export function SelectLite({
  ...
  // 键盘 ArrowUp/Down 改 highlight 后把高亮项滚进可视区 —— 长 dropdown 超过
  // maxHeight 280 时键盘用户能看到当前高亮。
  // 注意：不能用 scrollIntoView()，它会冒泡到祖先滚动容器（如设置弹窗的
  // ol-thinscroll），导致页面突然跳到顶部。改为手动计算 popover 内部滚动。
  useEffect(() => {
    if (!open || highlight < 0) return;
    if (highlightSource.current === 'mouse') return;
    ...
    if (!target) return;
    const cTop = popover.scrollTop;
    const cBottom = cTop + popover.clientHeight;
    const eTop = target.offsetTop;
    const eBottom = eTop + target.offsetHeight;
    if (eTop < cTop) {
      popover.scrollTop = eTop;
    } else if (eBottom > cBottom) {
      popover.scrollTop = eBottom - popover.clientHeight;
    }
  }, [highlight, open]);
  ...
  useEffect(() => {
    ...
    // trigger 的祖先滚动容器（如设置弹窗的 ol-thinscroll）滚动 → 也保留打开，
    // 避免用户在设置页面滚动时意外关闭下拉菜单。
    const handleScrollOutside = (event: Event) => {
      const target = event.target as Node | null;
      if (!target) return;
      // popover 内部滚动 → 保留打开
      if (popoverRef.current?.contains(target)) return;
      // trigger 的祖先滚动容器滚动 → 也保留打开
      const trigger = triggerRef.current;
      if (trigger && target instanceof HTMLElement) {
        const scrollableParent = target.closest('.ol-thinscroll, [style*="overflow"]');
        if (scrollableParent?.contains(trigger)) return;
      }
      closeMenu();
    };
  ...
  const openMenu = () => {
    ...
    setOpen(true);
    onOpenChange?.(true);
    // 设置全局标记，通知 scroll guard 跳过滚动位置恢复
    document.body.dataset.olSelectOpen = 'true';
  };
  ...
  const closeMenu = () => {
    ...
    window.setTimeout(() => {
      ...
      setAnchor(null);
      // 清除全局标记
      delete document.body.dataset.olSelectOpen;
    }, EXIT_ANIM_MS);
  };
    ...
    <>
      ...
      {open && anchor && createPortal(
        <div
          ref={setPopoverRef}
          role="listbox"
          onMouseLeave={() => { highlightSource.current = 'keyboard'; }}
          style={{
            position: 'fixed',
            left: anchor.left,
            top: anchor.top,
        ...
        >
          {options.map((option, index) => {
            if (option.separator) {
              return <hr key={option.value || `sep_${index}`} style={{ border: 0, borderTop: '0.5px solid var(--ol-line)', margin: '4px 6px' }} />;
            }
            ...
            return (
              <div
                ...
                onMouseEnter={() => { if (!option.disabled) { highlightSource.current = 'mouse'; setHighlight(index); } }}
                onMouseDown={event => {
                  event.preventDefault();
                  selectIndex(index);
                }}
